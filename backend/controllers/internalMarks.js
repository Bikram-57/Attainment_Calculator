  ...
  try {
    ...
    if (headerRowIndex === -1) {
      ...
      return res
        ...
        .json({
          message:
            "Could not find a valid header row containing Registration Numbers.",
        });
    }
    ...
    const subCode =
      extractTextValue(data[0], ["sub code", "subject code"]) || "SUBJECT_CODE";
    const subName =
      extractTextValue(data[0], ["sub name", "subject name"]) || "SUBJECT_NAME";
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const actualRegKey = Object.keys(row).find(
        (key) => key.toString().trim() === regNoHeaderName.toString().trim(),
      );
      ...
      const midTotal = extractValue(row, [
        "mid-sem",
        "midsem",
        "mid term",
        "mid exam",
      ...
      const [assign_CO1, assign_CO2, assign_CO3, assign_CO4, assign_CO5] =
        fillCOs(assignTotal, [1, 1, 1, 1, 1]);
      ...
      parsedStudents.push({
        ...
        marks: {
          ...
          Quiz_1_CO2: q1_CO2,
          Quiz_1_CO3: q1_CO3,
          Quiz_1_TOTAL: q1Total,
          Mid_Term_CO1: mid_CO1,
          Mid_Term_CO2: mid_CO2,
          Mid_Term_CO3: mid_CO3,
          Mid_Term_TOTAL: midTotal,
          Quiz_2_CO1: q2_CO1,
          Quiz_2_CO2: q2_CO2,
          Quiz_2_CO3: q2_CO3,
          Quiz_2_TOTAL: q2Total,
          Surprise_Quiz_CO1: sq_CO1,
          Surprise_Quiz_CO2: sq_CO2,
          Surprise_Quiz_CO3: sq_CO3,
          Surprise_Quiz_TOTAL: sqTotal,
          Assignment_CO1: assign_CO1,
          Assignment_CO2: assign_CO2,
          Assignment_CO3: assign_CO3,
          Assignment_CO4: assign_CO4,
          Assignment_CO5: assign_CO5,
          Assignment_TOTAL: assignTotal,
        },
      });
    }
    ...
    if (!subjectRecord) {
      ...
      subjectRecord.maxMarks.Quiz_1_CO1 = 2;
      subjectRecord.maxMarks.Quiz_1_CO2 = 2;
      subjectRecord.maxMarks.Quiz_1_CO3 = 1;
      subjectRecord.maxMarks.Quiz_1_TOTAL = 5;
      subjectRecord.maxMarks.Mid_Term_CO1 = 20;
      subjectRecord.maxMarks.Mid_Term_CO2 = 20;
      subjectRecord.maxMarks.Mid_Term_CO3 = 10;
      subjectRecord.maxMarks.Mid_Term_TOTAL = 50;
      subjectRecord.maxMarks.Quiz_2_CO1 = 2;
      subjectRecord.maxMarks.Quiz_2_CO2 = 2;
      subjectRecord.maxMarks.Quiz_2_CO3 = 1;
      subjectRecord.maxMarks.Quiz_2_TOTAL = 5;
      subjectRecord.maxMarks.Surprise_Quiz_CO1 = 2;
      subjectRecord.maxMarks.Surprise_Quiz_CO2 = 2;
      subjectRecord.maxMarks.Surprise_Quiz_CO3 = 1;
      subjectRecord.maxMarks.Surprise_Quiz_TOTAL = 5;
      subjectRecord.maxMarks.Assignment_CO1 = 1;
      subjectRecord.maxMarks.Assignment_CO2 = 1;
      subjectRecord.maxMarks.Assignment_CO3 = 1;
      subjectRecord.maxMarks.Assignment_CO4 = 1;
      subjectRecord.maxMarks.Assignment_CO5 = 1;
      subjectRecord.maxMarks.Assignment_TOTAL = 5;
    }

    // Merge internal marks into the specific student records
    for (const parsedStudent of parsedStudents) {
      const existingStudentIndex = subjectRecord.actualMarks.findIndex(
        (s) => s.regNo === parsedStudent.regNo,
      );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${subCode}_Internal_Mapped.xlsx"`,
    );
    res.setHeader(
      "Content-Type",
    ...
    res
      .status(500)
      .json({
        message: "Internal server error processing internal data",
        error: error.message,
      });
  }
};
...
const handleDownloadFormattedSheet = async (req, res) => {
  try {
    ...
    const endSemDoc = await EndSemMarks.findOne({
      $or: [{ subjectId: subjectId }, { subjectCode: subjectId }],
      academicYear,
    });
    ...
    if (!internalDoc && !endSemDoc) {
      return res.status(404).json({
        message: "No records found for this Subject ID and Academic Year.",
      });
    }
    ...
    if (internalDoc && Array.isArray(internalDoc.actualMarks)) {
    ...
    if (endSemDoc && Array.isArray(endSemDoc.students)) {
    ...
    const extMax = Number(endSemDoc?.maxMarksPerCO) || 20;
    ...
    const maxMarksRowNumber = maxMarksRow.number;

    worksheet.getCell(maxMarksRowNumber, 1).value = "Max Marks/CO";

    // Make absolutely sure there are no extra values
    // after column AC.
    for (let col = 30; col <= worksheet.columnCount; col++) {
      worksheet.getCell(maxMarksRowNumber, col).value = null;
    }
    ...
    worksheet.eachRow(
      ...
      (row, rowNumber) => {
        ...
        if (rowNumber === 1) {
          ...
          row.height = 22;
        } else if (rowNumber === maxMarksRowNumber) {
          row.height = 40;
        } else {
          row.height = 22;
        }
          ...
          (cell, colNumber) => {
            ...
            if (rowNumber === 1 || rowNumber === 2) {
              const group = columnGroups.find(
                (group) => colNumber >= group.start && colNumber <= group.end,
              );
            ...
            if (rowNumber === maxMarksRowNumber) {
              if (colNumber === 1) {
                cell.value = "Max Marks/CO";
    ...
    const finalRow = worksheet.getRow(maxMarksRowNumber);

    const finalValue = String(finalRow.getCell(1).value ?? "")
      .replace(/\r?\n/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    ...
    console.log("Max Marks/CO row:", maxMarksRowNumber);

    console.log("Max Marks/CO cell value:", JSON.stringify(finalValue));
    ...
    if (maxMarksRowNumber !== worksheet.rowCount) {
      throw new Error(
    ...
    console.error("Error generating consolidated sheet:", error);
