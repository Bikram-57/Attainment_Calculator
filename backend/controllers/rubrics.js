...
//         success: false,
//         message:
//           "Academic Year, Semester Type, and thresholds array are required.",
//       });
//     }
...
//     return res
//       .status(500)
//       .json({ success: false, message: "Server Error while saving rubric." });
//   }
// }


//updated format spacific
async function handleUploadrubrics(req, res) {
  try {
    const { academicYear, semesterType, thresholds } = req.body;
    ...
    // This regex requires exact format like "2023-2024"
    const academicYearRegex = /^\d{4}-\d{4}$/;
    if (!academicYearRegex.test(cleanAcademicYear)) {
      return res.status(400).json({
        success: false,
        message: "Invalid Academic Year format. Please use YYYY-YYYY (e.g., 2023-2024).",
      });
    }
    ...
    // 2. Fetch existing to check for duplicates
    const existingRubric = await Rubric.findOne({
      academicYear: cleanAcademicYear,
      semesterType: cleanSemesterType,
    });

    // --- FIX 2: Reject Duplicates ---
    if (existingRubric) {
      return res.status(409).json({
        success: false,
        message: `A rubric for the ${cleanSemesterType} semester of ${cleanAcademicYear} already exists.`,
      });
    }
    ...
    // 6. Handle MongoDB Duplicate Key Error (Fallback protection)
    if (error.code === 11000) {
      console.error("DUPLICATE KEY ERROR DETECTED:", error.keyValue);
      return res.status(409).json({
        success: false,
        message: "Database conflict: A rubric for this academic year and semester already exists.",
        errorDetails: error.keyValue,
      });
    }
...
async function handleUpdateRubrics(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Server Error while updating rubric." });
  }
}
...
async function handleFindAllRubrics(req, res) {
  try {
    ...
    const rubrics = await Rubric.find()
      .sort({ academicYear: -1, semesterType: 1 })
      .lean();

    if (!rubrics?.length) {
      return res
        .status(404)
        .json({ success: false, message: "No rubrics found in the database." });
    }
  ...
  try {
    ...
    if (!academicYear || !semesterType) {
      return res.status(400).json({
        success: false,
        message:
          "Please provide both the academicYear and the semesterType to delete.",
      });
    }
    ...
    return res.status(200).json({
      ...
      data: {
        academicYear: deletedRubric.academicYear,
        semesterType: deletedRubric.semesterType,
      },
    });
  } catch (error) {
    console.error("Error deleting rubric:", error);
...
const handleUploadRubricsThroughExcelSheet = async (req, res) => {
  try {
    ...
    if (!academicYear || !semesterType) {
      return res.status(400).json({
        error:
          "academicYear and semesterType are required in the request body.",
      });
    }
    ...
    const thresholds = rawExcelData.map((row, index) => {
      ...
      if (!("Level" in row) || !("Min %" in row) || !("Max %" in row)) {
        throw new Error(
          'Invalid file format. Columns must strictly be: "Level", "Min %", "Max %".',
        );
      }
    ...
    if (
      ...
      error.message.includes("invalid numeric data")
    ) {
      return res.status(400).json({ success: false, error: error.message });
    }
