...
//   // ==========================================
//     // SHEET 2: FINAL CO ATTAINMENT (Exact Match Format)
//     // ==========================================
//  // ==========================================
//     // SHEET 2: FINAL CO ATTAINMENT (Mapped to AttainmentTable)
//     // ==========================================
...
//       // Check if attainmentTable exists in the document
//       if (firstDoc.attainmentTable) {
          
//         // 1. STRICT COLUMNS (Visual headers mapped to your EXACT DB keys)
//         sheet2.columns = [
...
//         // 2. STYLE THE HEADER ROW (Light gray & Bold)
...
//         // 3. EXTRACT DATA FROM THE 'attainmentTable' OBJECT
//         const coRowsData = [];
//         const coNames = Object.keys(firstDoc.attainmentTable); // gets ["CO1", "CO2", "CO3", ...]

//         coNames.forEach(coName => {
//            const coData = firstDoc.attainmentTable[coName];
           
//            // Build a row matching the sheet2.columns keys
//            coRowsData.push({
//               coId: coName, // e.g. "CO1"
//               Quiz_1: coData.Quiz_1 !== undefined ? coData.Quiz_1 : '-',
//               Mid_Term: coData.Mid_Term !== undefined ? coData.Mid_Term : '-',
//               Quiz_2: coData.Quiz_2 !== undefined ? coData.Quiz_2 : '-',
//               Surprise_Quiz: coData.Surprise_Quiz !== undefined ? coData.Surprise_Quiz : '-',
//               Assignment: coData.Assignment !== undefined ? coData.Assignment : '-',
//               externalLevel: coData.externalLevel !== undefined ? coData.externalLevel : '-',
//               internalAvg: coData.internalAvg !== undefined ? coData.internalAvg : '-',
//               grandTotal: coData.grandTotal !== undefined ? coData.grandTotal : '-'
//            });
//         });
...
//           row.height = 25; // Add vertical padding
...
//         // 5. APPEND THE BOTTOM SUMMARY ROW
...
//         // Summary Label
//         const labelCell = finalRow.getCell(1);
//         labelCell.value = 'Final CO Attainment';
//         labelCell.alignment = { horizontal: 'right', vertical: 'middle', indent: 2 }; 
//         labelCell.font = { bold: true };
        
//         // Summary Value (Grabbed directly from your DB!)
//         const valueCell = finalRow.getCell(9);
//         valueCell.value = firstDoc.finalSubjectAttainment !== undefined ? firstDoc.finalSubjectAttainment : '-'; 
...
//         });
//       } else {
//          console.log("❌ No attainmentTable found inside the document.");
//       }
//     }
...
//       if (firstPoDoc.mappingData && firstPoDoc.averageCo) {
//           // 1. EXTRACT AND SORT DYNAMIC PO/PSO COLUMNS
//           const poKeys = Object.keys(firstPoDoc.averageCo).sort((a, b) => {
//               const numA = parseInt(a.replace(/[^0-9]/g, '')) || 0;
//               const numB = parseInt(b.replace(/[^0-9]/g, '')) || 0;
//               const textA = a.replace(/[0-9]/g, '');
//               const textB = b.replace(/[0-9]/g, '');
//               if (textA === textB) return numA - numB;
//               return textA.localeCompare(textB);
//           });

//           // 2. BUILD COLUMNS DYNAMICALLY
//           const cols = [{ header: "CO's", key: 'coId', width: 15 }];
//           poKeys.forEach(po => {
//               cols.push({ header: po.toUpperCase(), key: po, width: 12 });
//           });
//           sheet3.columns = cols;
...
//           // 4. MAP THE CO -> PO MAPPING DATA
//           const mappingRowsData = [];
//           const coNames = Object.keys(firstPoDoc.mappingData); // ["CO1", "CO2", "CO3"...]

//           coNames.forEach(co => {
//               const rowObj = { coId: co };
//               poKeys.forEach(po => {
//                   const val = firstPoDoc.mappingData[co][po];
//                   rowObj[po] = (val !== undefined && val !== null && val !== '') ? val : '-';
//               });
//               mappingRowsData.push(rowObj);
//           });

//           const dataRows = sheet3.addRows(mappingRowsData);
...
//           // 5. SUMMARY ROW 1: AVERAGE CO
//           const avgRowData = { coId: 'Average' };
//           poKeys.forEach(po => {
//               const val = firstPoDoc.averageCo[po];
//               avgRowData[po] = (val !== undefined && val !== null && val !== '') ? val : '-';
//           });
//           const avgRow = sheet3.addRow(avgRowData);
...
//           // 6. SUMMARY ROW 2: FINAL SUBJECT ATTAINMENT
//           const fsaRow = sheet3.addRow(['Final Subject Attainment']);
//           fsaRow.height = 30;
          
//           sheet3.mergeCells(fsaRow.number, 2, fsaRow.number, poKeys.length + 1);
          
//           const fsaLabel = fsaRow.getCell(1);
//           fsaLabel.font = { bold: true };
//           fsaLabel.alignment = { horizontal: 'center', vertical: 'middle' };
          
//           const fsaValue = fsaRow.getCell(2);
//           fsaValue.value = firstPoDoc.finalSubjectAttainment !== undefined ? firstPoDoc.finalSubjectAttainment : '-';
//           fsaValue.font = { bold: true };
//           fsaValue.alignment = { horizontal: 'center', vertical: 'middle' };

//           fsaRow.eachCell({ includeEmpty: true }, (cell) => {
...
//           // 7. SUMMARY ROW 3: FINAL PO ATTAINMENT (Bulletproof Fix)
...
//               // Fallback to case-insensitive match if direct key fails
//               if (val === undefined) {
//                   const lowerKey = Object.keys(poAttnData).find(k => k.toLowerCase() === po.toLowerCase());
//                   if (lowerKey) val = poAttnData[lowerKey];
//               }
//               poAttnRowData[po] = (val !== undefined && val !== null && val !== '') ? val : '-';
//           });
          
//           const poAttnRow = sheet3.addRow(poAttnRowData);
          
//           poAttnRow.height = 30;
//           poAttnRow.eachCell({ includeEmpty: true }, (cell) => {
//               cell.font = { bold: true };
//               cell.alignment = { horizontal: 'center', vertical: 'middle' };
...
//       } else {
//           console.log("❌ Missing mappingData or averageCo in PO document.");
//       }
//     }
