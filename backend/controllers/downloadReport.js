...
async function handleDownloadReport(req, res) {
    try {
        ...
        // PARALLEL FETCH: Fetch all 3 documents simultaneously using .findOne()
        ...
        if (coDoc?.attainmentTable) {
            ...
            sheet2.addRows(coRowsData).forEach(row => styleRow(row, { height: 25, border: STYLES.borderLight }));
...
async function handleDownloadCalculatedMarks(req, res) {
    try {
        ...
        if (!marksDoc?.actualMarks?.length) return res.status(404).json({ message: "No marks data found." });
        ...
        Object.keys(sampleMarks).forEach(key => {
            ...
            if (match) {
                const [, compName, coName] = match;
                if (!componentMap[compName]) { componentMap[compName] = []; orderedComponents.push(compName); }
                componentMap[compName].push(coName);
            }
        });
        ...
        sheet1.getRow(1).alignment = STYLES.center; sheet1.getRow(1).font = STYLES.bold;
        sheet1.getRow(2).alignment = STYLES.center; sheet1.getRow(2).font = STYLES.bold;
        ...
        sheet1.addRows(studentRows).forEach(row => styleRow(row, { height: 25, border: STYLES.borderLight }));
        ...
        sheet1.addRows(calcRowsData).forEach(row => styleRow(row, { height: 25, font: STYLES.bold, fillArgb: 'FFF2F2F2', border: STYLES.borderLight }));
