...
const requestOtp = async (req, res) => {
  try {
    ...
    // Sanitize input to prevent case-sensitivity login bugs
    const cleanEmail = email.trim().toLowerCase();
    const user = await User.findOne({ email: cleanEmail });

    // Security: Generic success message prevents attackers from guessing registered emails
    if (!user) {
    ...
    await sendEmail(user.email, "Password Reset Verification Code", `${otp}`);
...
const verifyOtp = async (req, res) => {
  try {
    ...
    // 1. Verify user exists and actually has an active OTP process
    if (!user?.resetOtp || !user?.resetOtpExpires) {
      return res.status(400).json({ message: "Invalid request or OTP process not initiated." });
    }

    // 2. Check if the 10-minute window has expired
    if (Date.now() > user.resetOtpExpires) {
      // Cleanup the expired OTP to keep the database clean
    ...
    // 3. Compare the raw 6-digit code with the hashed version (trim to prevent whitespace bugs)
    const isMatch = await bcrypt.compare(otp.toString().trim(), user.resetOtp);
    if (!isMatch) {
      return res.status(400).json({ message: "Invalid OTP code" });
    }

    // 4. Generate the temporary "Golden Ticket" valid for 5 mins
    ...
    // 5. SECURITY: Clear the OTP immediately upon successful verification 
    // This prevents attackers from reusing the OTP within the 10-minute window
    user.resetOtp = undefined;
    user.resetOtpExpires = undefined;
    await user.save();
...
const resetPassword = async (req, res) => {
  try {
    ...
    // 3. Failsafe: Ensure OTP fields are clear
    user.resetOtp = undefined;
    user.resetOtpExpires = undefined;
    await user.save();
